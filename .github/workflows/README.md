...
After you approve, tests **do** run the customer SHA with GKE (`GKE_SERVICE_ACCOUNT_JSON_JFROG_DEV`, `WHITELIST_CIDR_JFROG_DEV`, GKE zone/project/machine vars) and platform-license secrets. That is required to test their code. Treat the approval as a trust decision: do not approve a PR that looks like it will dump env or exfiltrate credentials.
...
A typical customer PR already adds a `CHANGELOG.md` bullet for the resource they changed. That is a normal PR file. **Merging the PR lands that bullet.** CI does not write it.

### 2. CI “Tested on JFrog Platform …” heading (we cannot push to a fork)

The old job rewrote the version heading and `git push`ed to the PR branch. `GITHUB_TOKEN` cannot write to `customer/terraform-provider-bridge`.

The heading CI writes is: `Tested on JFrog Platform $PLATFORM_VERSION (Artifactory $ARTIFACTORY_VERSION)` plus Terraform and OpenTofu CLI versions.

| PR type | After tests pass |
|---|---|
| Same-repo (`jfrog/terraform-provider-bridge`) | CI still updates `CHANGELOG.md` and pushes, as before |
...
These must be done on `jfrog/terraform-provider-bridge` after this lands on `main`:
...
2. Leave test secrets/vars on **`development`** (or repo/org secrets), including GKE credentials and platform license.
